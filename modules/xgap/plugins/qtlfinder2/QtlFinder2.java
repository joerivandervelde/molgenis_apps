...
public class QtlFinder2 extends PluginModel<Entity>
{
		...
		if (request.getString("__action") != null)
		{
			...
			try
			{
				...
				if (action.equals("shop"))
				{
					...
					// Entity e = db.findById(ObservationElement.class,
					// shopMeId); cannot use this: problem with lists and java
					// generics when load()
				...
				if (action.startsWith("__entity__report__for__"))
				{
					...
					// we expect 1 hit exactly: ObservationElement names are
					// unique, and since they have already been found, it should
					// exist (unless deleted in the meantime..)
		...
		else
		{
			...
			if (permaLinkIds != null)
			{
				try
				{
					...
					// special special: WormBaseID is given (only 1 for the
					// moment
					...
					else
					{
						...
						List<? extends Entity> findIds = db.find(entityClass, new QueryRule(ObservationElement.ID,
								Operator.IN, ids));
			...
			throws DatabaseException
	{
		...
		// keep a list of genes for which the probes are shown to make the GO
		// term info box
		Map<String, Gene> probeToGene = new HashMap<String, Gene>();

		// additional: include the Gene objects for the shopping cart view so
		// they are not lost on next search action
		...
		if (this.model.getProbeToGene() != null && this.model.getProbeToGene().size() > 0)
		{
			...
			for (Entity e : this.model.getShoppingCart().values())
			{
				...
				// if the the ProbeToGene list contains the shopping cart item
				// (probe name), add it to the new ProbeToGene list
		...
		outer: for (String name : hits.keySet())
		{
			...
			// e.get(Field.TYPE_FIELD).toString() + " " +
			// e.get(ObservationElement.NAME).toString());
			...
			{
				result.put(e.get(ObservationElement.NAME).toString(), e);
				// System.out.println("GENE2PROBE ADDING ALSO: "
				// +e.get(ObservationElement.NAME).toString());
				nrOfResults++;
				if (nrOfResults == limit)
				{
	...
	private Map<String, Entity> query(Class entityClass, Database db, String query, int limit) throws DatabaseException
	{
		...
		for (Entity e : result)
		{
			...
			// System.out.println("REGULAR SEARCH HIT: " +
			// e.get(ObservationElement.NAME));
	...
	private QTLMultiPlotResult multiplot(List<Entity> entities, Database db) throws Exception
	{
		...
		File tmpData = new File(System.getProperty("java.io.tmpdir") + File.separator + "rplot_data_table_"
				+ System.nanoTime() + ".txt");
		File cytoscapeNetwork = new File(System.getProperty("java.io.tmpdir") + File.separator + "cyto_qtl_network_"
				+ System.nanoTime() + ".txt");
		File cytoscapeNodes = new File(System.getProperty("java.io.tmpdir") + File.separator + "cyto_node_attr_"
				+ System.nanoTime() + ".txt");
		...
		for (Data d : allData)
		{
			...
			try
			{
				...
				// TODO: limited, could also be SNP or another potential
				// subclass
				if ((d.getTargetType().equals("Marker") || d.getFeatureType().equals("Marker")))
				{
					...
					for (Entity e : entities)
					{
						...
						// matrix may not have the trait type on rows AND
						// columns (this is correlation data or so)
						...
						// one of the matrix dimensions must match the type of
						// the trait
						if (d.getTargetType().equals(e.get(Field.TYPE_FIELD))
								|| d.getFeatureType().equals(e.get(Field.TYPE_FIELD)))
						{
							...
							// get the basepair position of the trait and
							// chromosome name, if possible
							long locus;
							String traitChrName;
							...
							name = e.get(ObservableFeature.NAME).toString()
									+ (e.get(ObservableFeature.LABEL) != null ? "/"
											+ e.get(ObservableFeature.LABEL).toString() : "");

							// iterate over the markers and write out input
							// files for R / CytoScape
							for (int markerIndex = 0; markerIndex < markerNames.size(); markerIndex++)
							{
								if (nameToMarker.containsKey(markerNames.get(markerIndex)))
								{
									String chrId = nameToMarker.get(markerNames.get(markerIndex)).getChromosome_Id() != null ? nameToMarker
											.get(markerNames.get(markerIndex)).getChromosome_Id().toString()
											: "-";

									// query and store chromosome objects for
									// this name if it's not there yet
									String chrName = nameToMarker.get(markerNames.get(markerIndex))
											.getChromosome_Name();
									if (!usedChromosomes.containsKey(chrName))
									{
										Chromosome c = db.find(Chromosome.class,
												new QueryRule(Chromosome.NAME, Operator.EQUALS, chrName)).get(0);
										usedChromosomes.put(chrName, c);
									}

									// index, marker, chrId, markerLoc, trait,
									// traitLoc, LODscore, dataId
									bw.write(overallIndex + " \"" + markerNames.get(markerIndex) + "\" " + chrId + " "
											+ nameToMarker.get(markerNames.get(markerIndex)).getBpStart() + " \""
											+ name + "\" " + locus + " " + Dvalues[markerIndex] + " " + d.getId());
									bw.newLine();

									if (Dvalues[markerIndex].doubleValue() > 3.5)
									{
										// trait, "QTL", marker, LODscore
										bw_network.write(name + "\t" + "QTL" + "\t" + markerNames.get(markerIndex)
												+ "\t" + Dvalues[markerIndex]);
										bw_network.newLine();

										String marker_chrName = nameToMarker.get(markerNames.get(markerIndex))
												.getChromosome_Name() != null ? nameToMarker.get(
												markerNames.get(markerIndex)).getChromosome_Name() : "-";
										if (!nodesInFile.contains(name))
										{
											// trait, traitChr, traitLocus,
											// dataName
											bw_nodes.write(name + "\t" + "trait" + "\t" + traitChrName + "\t" + locus
													+ "\t" + d.getName());
											bw_nodes.newLine();
										...
										{
											// marker, markerChr, markerLocus,
											// dataName
											bw_nodes.write(markerNames.get(markerIndex) + "\t" + "marker" + "\t"
													+ marker_chrName + "\t"
													+ nameToMarker.get(markerNames.get(markerIndex)).getBpStart()
													+ "\t" + d.getName());
											bw_nodes.newLine();
